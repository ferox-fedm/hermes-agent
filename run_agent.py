...
class AIAgent:
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        REST preload is disabled by default to prevent duplicate model
        instances from concurrent JIT loading. Set HERMES_LMSTUDIO_PRELOAD=1
        to restore preload; ``model.lmstudio_load_mode: jit`` also forces JIT.
        """
        ...
        if (getattr(self, "lmstudio_load_mode", "explicit") or "explicit").strip().lower() == "jit":
            ...
            return None
        if os.environ.get("HERMES_LMSTUDIO_PRELOAD", "").strip().lower() not in ("1", "true", "yes", "on"):
            logger.debug(
