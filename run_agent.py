...
class AIAgent:
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Optionally preload the LM Studio model with at least Hermes' minimum context.

        The default is intentionally disabled. LM Studio's JIT loader can load
        the same model as a distinct runtime when this REST preload path asks
        for a different context length than the OpenAI-compatible request path
        would use. In a one-loaded-model setup, that produces duplicate
        same-model instances instead of the expected JIT queue/evict behavior.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        if not os.environ.get("HERMES_LMSTUDIO_PRELOAD", "").strip() in ("1", "true", "yes", "on"):
            logger.debug(
                "LM Studio REST preload disabled; relying on JIT model loading "
                "(set HERMES_LMSTUDIO_PRELOAD=1 to restore old behavior)"
            )
            return
        try:
