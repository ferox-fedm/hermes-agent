...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        try:
            ...
            # Text output behavioral dedup: detect repeated assistant
            # text across turns (post-compression behavioral loops).
            # Skip truncated/incomplete responses to avoid false positives.
            assistant_text = assistant_message.content or ""
            if (
                hasattr(agent, '_text_output_guardrails')
                and assistant_text
                and finish_reason not in ("length", "incomplete")
            ):
                td = agent._text_output_guardrails.observe(assistant_text)
                if td.action == "warn":
                    agent._emit_warning(td.message)
            ...
            if assistant_message.tool_calls:
                ...
                # ── Task terminal: kanban_complete / kanban_block ────
                # When a worker successfully completes or blocks its task,
                # it should NOT continue taking further action. This prevents
                # non-reasoning models from generating additional tool calls
                # after finishing their work, which causes protocol violations,
                # loop states, and wasted tokens. (#41236)
                if getattr(agent, "_task_terminal", False):
                    _turn_exit_reason = "task_terminal"
                    agent._emit_status("✅ Task complete — stopping agent")
                    break
                ...
                # ── Task terminal check in text-only path ────
                # If a previous turn called kanban_complete/kanban_block,
                # don't waste retries — just exit immediately. (#41236)
                if getattr(agent, "_task_terminal", False):
                    _turn_exit_reason = "task_terminal"
                    agent._emit_status("✅ Task complete — stopping agent")
                    final_response = final_response or "Task was completed in a previous turn."
                    break
