...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    # ------------------------------------------------------------------
    # Deterministic tool-activity snapshot
    # ------------------------------------------------------------------

    @staticmethod
    def _tool_activity_key_desc(tool_name: str, args: Any) -> tuple[str, str]:
        """Return (dedup_key, one_line_description) for a tool call.

        Handles both OpenAI ``function`` dict format and the normalised
        ``{name, arguments}`` shape.
        """
        target = ""
        if tool_name in ("write_file", "mcp_filesystem_write_file"):
            target = args.get("file_path", args.get("path", ""))
        elif tool_name in ("patch",):
            target = args.get("file_path", args.get("path", ""))
        elif tool_name in ("terminal",):
            target = args.get("command", "")[:80]
        elif tool_name in ("read_file", "mcp_filesystem_read_file",
                           "mcp_filesystem_read_text_file"):
            target = args.get("file_path", args.get("path", ""))
        elif tool_name in ("search_files", "mcp_filesystem_search_files"):
            target = args.get("pattern", args.get("query", ""))
        elif tool_name in ("web_search",):
            target = args.get("query", "")[:60]
        elif tool_name in ("web_extract",):
            target = args.get("url", "")[:60]
        elif tool_name in ("browser_navigate",):
            target = args.get("url", args.get("target", ""))[:60]

        key = f"{tool_name}:{target.lower().strip()}"

        # Build a one-line description
        if tool_name == "write_file":
            desc = f"wrote {target}" if target else "wrote file"
        elif tool_name == "patch":
            desc = f"patched {target}" if target else "patched file"
        elif tool_name == "terminal":
            cmd = (args.get("command", "") or "")[:60]
            desc = f"ran: {cmd}" if cmd else "ran terminal command"
        elif tool_name in ("read_file", "mcp_filesystem_read_file",
                           "mcp_filesystem_read_text_file"):
            desc = f"read {target}" if target else "read file"
        elif tool_name in ("search_files", "mcp_filesystem_search_files"):
            desc = f"searched for {target}" if target else "searched files"
        elif tool_name == "web_search":
            desc = f"web search: {target}" if target else "web search"
        elif tool_name == "web_extract":
            desc = f"extracted {target}" if target else "web extract"
        elif tool_name == "browser_navigate":
            desc = f"navigated to {target}" if target else "browser navigate"
        else:
            desc = f"{tool_name}({target})" if target else tool_name

        return key, desc

    def _build_tool_activity_snapshot(
        self,
        messages: List[Dict[str, Any]],
        compress_start: int,
        compress_end: int,
        ...
        """Build a deterministic snapshot of recent tool activity from the
        messages that are about to be compressed away.

        Returns a markdown section suitable for injection into the summary.
        """
        ...
        for i in range(compress_start, compress_end):
            ...
            if msg.get("role") != "assistant":
                continue
            for tc in (msg.get("tool_calls") or []):
                # Support both OpenAI dict and normalised object formats
                if isinstance(tc, dict):
                ...
                key, desc = self._tool_activity_key_desc(tool_name, args)
                if key not in seen and desc:
                    seen.add(key)
                    all_bullets.append(desc)

        if not all_bullets:
            return ""

        # Keep only the most recent max_items, then reverse for chronological display
        recent = all_bullets[-max_items:]
        recent.reverse()
        return "## Completed Tool Activity\n" + "\n".join(
            f"- {b}" for b in recent
        ) + "\n"
    ...
    def compress(self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None, force: bool = False) -> List[Dict[str, Any]]:
        ...
        for i in range(compress_start):
            ...
            if i == 0 and msg.get("role") == "system":
                ...
                _compression_note = "[Note: Earlier turns were compacted. The summary includes completed work and a deterministic tool-activity snapshot. The current filesystem, git state, and test results reflect this progress. DO NOT re-plan, re-explain, or re-execute items already listed. Continue forward from the current state. Persistent memory (MEMORY.md, USER.md) remains authoritative.]"
        ...
        # Inject deterministic tool-activity snapshot into the summary
        # so the agent always knows what concrete work was done,
        # regardless of LLM summary quality.
        tool_snapshot = self._build_tool_activity_snapshot(
