...
from agent.lmstudio_single_model import (
    create_chat_completion_with_lmstudio_gate,
    async_create_chat_completion_with_lmstudio_gate,
)
    ...
    *,
    ...
    return _validate_llm_response(
        create_chat_completion_with_lmstudio_gate(
            ...
            purpose=f"auxiliary:{task or 'call'}:retry",
            logger=logger,
        ), task,
    )
...
async def _retry_same_provider_async(
    *,
    ...
    if _is_anthropic_compat_endpoint(resolved_provider, retry_base):
        retry_kwargs["messages"] = _convert_openai_images_to_anthropic(retry_kwargs["messages"])
    return _validate_llm_response(
        await async_create_chat_completion_with_lmstudio_gate(
            retry_client,
            retry_kwargs,
            provider=resolved_provider,
            base_url=retry_base or resolved_base_url,
        ...
        try:
            return _validate_llm_response(
                create_chat_completion_with_lmstudio_gate(
                    ...
                    base_url=_base_info or resolved_base_url,
                    model=final_model,
                    purpose=f"auxiliary:{task or 'call'}",
                    logger=logger,
                ), task)
            ...
            if fb_client is not None:
                ...
                return _validate_llm_response(
                    create_chat_completion_with_lmstudio_gate(
                        ...
                        base_url=str(getattr(fb_client, "base_url", "") or ""),
                        model=fb_model,
                        purpose=f"auxiliary:{task or 'call'}:fallback",
                        logger=logger,
                    ), task)
