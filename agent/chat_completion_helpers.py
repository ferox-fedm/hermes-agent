...
def _reset_stale_streak(agent) -> None:
    """Reset the cross-turn stale-call circuit breaker counter (#58962).

    Called when the provider is being swapped (model switch, fallback
    activation, primary restore) so the new provider gets a fresh
    stream attempt instead of inheriting a latched streak from the
    previous one.
    """
    agent._consecutive_stale_streams = 0


def rewrite_prompt_model_identity(agent, new_model: str, new_provider: str) -> None:
    """Rewrite ``Model:``/``Provider:`` identity lines in the cached system prompt.

    When provider failover swaps the runtime, the agent reports the model that
    is actually answering.  Only the LAST occurrence of each line is rewritten —
    earlier occurrences may be user content (memory snapshots, context files)
    ...
    if last_model_idx is not None:
        lines[last_model_idx] = f"Model: {new_model}"
    if last_provider_idx is not None:
        lines[last_provider_idx] = f"Provider: {new_provider}"
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # After N consecutive stale kills, short-circuit immediately instead
    # of re-waiting out the full timeout.  The breaker latches: once
    # triggered, every subsequent call raises instantly until the streak
    # is reset (by model switch, fallback activation, or successful
    # response).
    _giveup = int(os.getenv("HERMES_STREAM_STALE_GIVEUP", "5"))
    if _giveup > 0:
        _streak = getattr(agent, "_consecutive_stale_streams", 0)
        if _streak >= _giveup:
            raise RuntimeError(
                f"Provider has been unresponsive for {_streak} consecutive "
                f"stale attempts (threshold: {_giveup}). Aborting this call. "
                "Switch models (/model) or retry later."
            )
    ...
    def _call():
        try:
            if agent.api_mode == "codex_responses":
                ...
                result["response"] = create_chat_completion_with_lmstudio_gate(
                    request_client,
                    api_kwargs,
                    provider=agent.provider,
                    base_url=agent.base_url,
                    model=api_kwargs.get("model", agent.model),
                    purpose="chat_completion_request",
                    logger=logger,
                )
    ...
    while t.is_alive():
        ...
        if _elapsed > _stale_timeout:
            ...
            # Increment the stale-stream streak for the circuit breaker.
            agent._consecutive_stale_streams = getattr(
                agent, "_consecutive_stale_streams", 0
            ) + 1
            break
    ...
    if result["error"] is not None:
        raise result["error"]
    # Successful response — reset the stale-stream streak.
    agent._consecutive_stale_streams = 0
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        # Reset the stale-call streak so the new provider gets a fresh
        # attempt instead of inheriting a latched breaker from the old one.
    ...
    try:
        ...
        if final_response:
            ...
            if agent.api_mode == "codex_responses":
                ...
                summary_client = agent._ensure_primary_openai_client(reason="iteration_limit_summary_retry")
                summary_response = create_chat_completion_with_lmstudio_gate(
                    summary_client,
                    summary_kwargs,
                    provider=agent.provider,
                    base_url=agent.base_url,
                    model=summary_kwargs.get("model", agent.model),
                    purpose="iteration_limit_summary_retry",
                    logger=logger,
                )
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions():
        ...
        stream = create_chat_completion_with_lmstudio_gate(
            ...
            purpose="chat_completion_stream_request",
            logger=logger,
        )
