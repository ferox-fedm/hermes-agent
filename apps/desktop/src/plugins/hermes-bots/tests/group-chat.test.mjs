...
test('source contract: workspace header offers disband behind a ConfirmDialog', () => {
  ...
  // Fork feature (8265b873e): the header action opens the ConfirmDialog; the
  // label names the exact group rather than pointing at a removed session
  // browser. Adapted post-upstream-merge: upstream renders the same wording
  // as a `label:` prop (it was `title:` in the fork's pre-merge plugin).
